...
namespace cYo.Projects.ComicRack.Viewer.Views
{
    public partial class ComicBrowserControl : SubView, IComicBrowser, IGetBookList, IRefreshDisplay, ISearchOptions, IDisplayWorkspace, IItemSize, ISettingsChanged
    {
        ...
        public void SetExtraColumns(ExtraColumnType extraColumnType = ExtraColumnType.All)
        {
            if (extraColumnType.HasFlag(ExtraColumnType.Custom))
                SetCustomColumns();

            if (extraColumnType.HasFlag(ExtraColumnType.VirtualTag))
        ...
        public ComicBrowserControl()
        {
            ...
            itemView.Columns.Add(new ItemViewColumn(71, "Published (Regional)", 40, new ComicListField("PublishedRegional", "Publication date of the Book (regional formating)", null, StringTrimming.Word, typeof(DateTime)), new CoverViewItemBookComparer<ComicBookPublishedComparer>(), new CoverViewItemBookGrouper<ComicBookGroupPublished>(), visible: false, StringAlignment.Far, strings));
        ...
        private bool MoveBooks(IEnumerable<ComicBook> books, bool bottom)
        {
            ...
            {
                foreach (ComicBook item in source.AsEnumerable().Reverse())
                {
